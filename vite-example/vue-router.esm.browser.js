...
function resolveAsyncComponents (matched) {
  return (to, from, next) => {
    ...
    flatMapComponents(matched, (def, match) => {
...
function extractGuards (
  ...
  const guards = records.reduce((fns, { component }) => {
    const cpt = component;
    const pageConfig = cpt && cpt.pageConfig;

    // 组件上是否有相应的守卫函数
    let guard = cpt && cpt[name];
    if (!guard && pageConfig) {
      ...
      // 如果有 redirect 字段则转化会 beforeRouteEnter 守卫进行跳转
        ...
        guard = (to, from, next) => next(pageConfig.redirect);
      }
    }
    if (guard) {
      fns.push(guard);
